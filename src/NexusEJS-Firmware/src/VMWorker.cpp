...
void VMWorker::vtScheduleNext()
{
	...
	while (!vtCanScheduleCheck(VirtualThreads[vtSchedIndex])) {
		...
		if (VirtualThreads.size() == 0) {
			return; 
			//如果此时已经没有任何虚拟线程可以调度，直接退出，让VMWorker自动结束自身
		}

		vtSchedIndex = (vtSchedIndex + 1) % VirtualThreads.size();

		if (vtSchedIndex == scanIndexStart) {
			platform.ThreadSleep(VT_SCHED_IDLE_SLEEP_TIME_MS);
		}
	}

	//重置计数器，开启下一个时间片
	vtInstrCounter = VT_SCHED_INSTRUCTION_CNT;
}

bool VMWorker::deadCheck()
{
...
void VMWorker::ThrowError(VariableValue& messageString)
{
	...
	auto& callFrames = getCurrentCallingLink();
...
static VariableValue __make_closure(VariableValue& function, FuncFrame* frame, VMWorker* worker) {
	...
	//查找哪些局部变量需要被捕获
	uint32_t index = 0;
	for (uint16_t varbNameId : frame->localVarNames) {
		//计算交集进行捕获
		for (uint16_t symid : outsideSym) {
			if (symid == varbNameId) {
				//自动装箱闭包捕获的变量（如果是值类型）
				if (frame->localVariables[index].varType != ValueType::REF) {
					frame->localVariables[index] = MakeBoxedValue(frame->localVariables[index],worker);
				}
...
static VariableValue* __vmworker_find_variable(VMWorker& worker, std::string& name) {
	auto& callingStack = worker.getCurrentCallingLink();
...
VariableValue VMWorker::VMWorkerTask() {
	...
	while (true) {
		...
		FuncFrame* currentFn = &callFrames.back();
		...
		*/
		...
		if (!needResetLoop) { //如果发生需要重置的操作（比如异常抛出）则不步进
