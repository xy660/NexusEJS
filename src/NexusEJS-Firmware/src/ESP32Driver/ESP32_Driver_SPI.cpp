...
enum NEXUS_SPIBitOrder {
    NEXUS_SPI_MSBFIRST = 0,
    NEXUS_SPI_LSBFIRST
};
...
int get_nexus_spi_mode(int mode) {
    ...
    switch(mode) {
        case NEXUS_SPI_MODE_0: return 0;  // SPI_MODE0
        case NEXUS_SPI_MODE_1: return 1;  // SPI_MODE1
        case NEXUS_SPI_MODE_2: return 2;  // SPI_MODE2
        case NEXUS_SPI_MODE_3: return 3;  // SPI_MODE3
        default: return 0;
    }
}
...
spi_host_device_t get_spi_host(int bus_num) {
    switch(bus_num) {
        case 1: return SPI2_HOST;  // HSPI对应SPI2_HOST
        case 2: return SPI3_HOST;  // VSPI对应SPI3_HOST
        default: return SPI2_HOST;  // 默认为HSPI
    }
}
...
void ESP32_SPI_Init(VM* VMInstance){
    ...
    // 添加常量定义 - 使用NEXUS_前缀
    spiClass->implement.objectImpl[L"MODE_0"] = CreateNumberVariable(NEXUS_SPI_MODE_0);
    spiClass->implement.objectImpl[L"MODE_1"] = CreateNumberVariable(NEXUS_SPI_MODE_1);
    spiClass->implement.objectImpl[L"MODE_2"] = CreateNumberVariable(NEXUS_SPI_MODE_2);
    spiClass->implement.objectImpl[L"MODE_3"] = CreateNumberVariable(NEXUS_SPI_MODE_3);
    spiClass->implement.objectImpl[L"MSBFIRST"] = CreateNumberVariable(NEXUS_SPI_MSBFIRST);
    spiClass->implement.objectImpl[L"LSBFIRST"] = CreateNumberVariable(NEXUS_SPI_LSBFIRST);
    spiClass->implement.objectImpl[L"HSPI"] = CreateNumberVariable(1);  // HSPI总线
    spiClass->implement.objectImpl[L"VSPI"] = CreateNumberVariable(2);  // VSPI总线
    
    // 初始化SPI主机
    spiClass->implement.objectImpl[L"init"] = VM::CreateSystemFunc(6,
    [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
        if(args[0].getContentType() != ValueType::NUM ||  // bus_num
           ...
           args[5].getContentType() != ValueType::NUM){   // data_bits
            currentWorker->ThrowError(L"Invalid argument type: expected (bus_num, cs_pin, clock_speed, mode, bit_order, data_bits)");
        ...
        if(bus_num != 1 && bus_num != 2){
            currentWorker->ThrowError(L"Bus number must be 1 (HSPI) or 2 (VSPI)");
            return VariableValue();
        }
        
        if(clock_speed < 100000 || clock_speed > 40000000){
            currentWorker->ThrowError(L"Clock speed must be between 100KHz and 40MHz");
            return VariableValue();
        }
        
        if(mode < NEXUS_SPI_MODE_0 || mode > NEXUS_SPI_MODE_3){
            currentWorker->ThrowError(L"Mode must be between 0 and 3");
            return VariableValue();
        }
        
        if(data_bits != 8 && data_bits != 16 && data_bits != 32){
            currentWorker->ThrowError(L"Data bits must be 8, 16, or 32");
            return VariableValue();
        }
        
        // 创建SPI对象
        VMObject* spiObject = currentWorker->VMInstance->currentGC->GC_NewObject(ValueType::OBJECT);
        ...
        objContainer[L"bus"] = CreateNumberVariable(bus_num);
        objContainer[L"cs"] = CreateNumberVariable(cs_pin);
        objContainer[L"clock"] = CreateNumberVariable(clock_speed);
        objContainer[L"mode"] = CreateNumberVariable(mode);
        objContainer[L"bit_order"] = CreateNumberVariable(bit_order);
        objContainer[L"data_bits"] = CreateNumberVariable(data_bits);
        ...
        if(bit_order == NEXUS_SPI_LSBFIRST) {
            dev_config.flags = SPI_DEVICE_TXBIT_LSBFIRST | SPI_DEVICE_RXBIT_LSBFIRST;
        }
        ...
        if(ret != ESP_OK){
            char error_msg[100];
            snprintf(error_msg, sizeof(error_msg), "Failed to initialize SPI bus: %d", ret);
            currentWorker->ThrowError(std::wstring(error_msg, error_msg + strlen(error_msg)));
            return VariableValue();
        }
        ...
        if(ret != ESP_OK){
            spi_bus_free(host);
            char error_msg[100];
            snprintf(error_msg, sizeof(error_msg), "Failed to add SPI device: %d", ret);
            currentWorker->ThrowError(std::wstring(error_msg, error_msg + strlen(error_msg)));
            return VariableValue();
        }
        
        // 存储SPI实例
        SPIInstance spi_inst = {
        ...
        // 传输数据（全双工）
        objContainer[L"transfer"] = VM::CreateSystemFunc(2, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            uint32_t instanceId = (uint32_t)thisValue->implement.objectImpl[L"_id"].content.number;
            
            if(spiInstances.find(instanceId) == spiInstances.end()){
                currentWorker->ThrowError(L"SPI instance not initialized or already released");
                return VariableValue();
            }
            
            SPIInstance& spi_inst = spiInstances[instanceId];
            
            if(args[0].getContentType() != ValueType::OBJECT || args[1].getContentType() != ValueType::OBJECT){
            ...
            // 执行SPI传输
            esp_err_t ret = spi_master_transmit(spi_inst.device_handle, txBuffer.data, rxBuffer.data, txBuffer.length,rxBuffer.length);
            
            if(ret != ESP_OK){
                char error_msg[100];
                snprintf(error_msg, sizeof(error_msg), "SPI transfer failed: %d", ret);
        ...
        objContainer[L"write"] = VM::CreateSystemFunc(1, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            uint32_t instanceId = (uint32_t)thisValue->implement.objectImpl[L"_id"].content.number;
            ...
            if(args[0].getContentType() == ValueType::NUM){
                ...
                if(obj.find(L"bufid") != obj.end()){
                    ...
                    // 执行SPI发送
                    esp_err_t ret = spi_master_transmit(spi_inst.device_handle, bufinfo.data, nullptr, bufinfo.length,0);
                    if(ret != ESP_OK) printf("SPI write failed: %d", ret);
                    return CreateBooleanVariable(ret == ESP_OK);
                }
                else {
                    currentWorker->ThrowError(L"Data argument must be a number or Buffer");
                    return VariableValue();
                }
            } else {
                currentWorker->ThrowError(L"Data argument must be a number or Buffer");
            ...
            if(tx_data.empty()){
                currentWorker->ThrowError(L"No data to write");
                return VariableValue();
            }
            
            // 执行SPI发送
            esp_err_t ret = spi_master_transmit(spi_inst.device_handle, tx_data.data(), nullptr, tx_data.size(),0);
        ...
        objContainer[L"read"] = VM::CreateSystemFunc(2, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            if(args[0].getContentType() != ValueType::OBJECT || args[1].getContentType() != ValueType::NUM){
            ...
            if(length <= 0 || length > 4096){
                currentWorker->ThrowError(L"Read length must be between 1 and 4096");
                return VariableValue();
            }
            ...
            // 执行SPI传输
            esp_err_t ret = spi_master_transmit(spi_inst.device_handle, tx_data.data(), buf_info.data, length,length);
        ...
        // 传输16位数据
        objContainer[L"transfer16"] = VM::CreateSystemFunc(1, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            if(args[0].getContentType() != ValueType::NUM){
                currentWorker->ThrowError(L"Invalid argument: expected 16-bit data");
                return VariableValue();
            }
        ...
        // 传输32位数据
        objContainer[L"transfer32"] = VM::CreateSystemFunc(1, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            if(args[0].getContentType() != ValueType::NUM){
                currentWorker->ThrowError(L"Invalid argument: expected 32-bit data");
                return VariableValue();
            }
        ...
        // 设置CS引脚状态
        objContainer[L"setCS"] = VM::CreateSystemFunc(1, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            if(args[0].getContentType() != ValueType::BOOL){
                currentWorker->ThrowError(L"Invalid argument: expected boolean");
                return VariableValue();
            }
        ...
        // 获取SPI配置信息
        objContainer[L"getConfig"] = VM::CreateSystemFunc(0, [](std::vector<VariableValue>& args, VMObject* thisValue, VMWorker* currentWorker) -> VariableValue {
            uint32_t instanceId = (uint32_t)thisValue->implement.objectImpl[L"_id"].content.number;
            ...
            VMObject* configObj = currentWorker->VMInstance->currentGC->GC_NewObject(ValueType::OBJECT);
            auto& configContainer = configObj->implement.objectImpl;
            
            configContainer[L"bus"] = CreateNumberVariable(spi_inst.host == SPI2_HOST ? 1 : 2);
            configContainer[L"cs_pin"] = CreateNumberVariable(spi_inst.cs_pin);
            configContainer[L"clock_speed"] = CreateNumberVariable(spi_inst.clock_speed);
            configContainer[L"mode"] = CreateNumberVariable(spi_inst.mode);
            configContainer[L"bit_order"] = CreateNumberVariable(spi_inst.bit_order);
            configContainer[L"data_bits"] = CreateNumberVariable(spi_inst.data_bits);
    ...
    std::wstring spiClassName = L"SPI";
