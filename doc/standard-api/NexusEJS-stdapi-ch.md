...
- *vtSetScheduleEnabled(enable: boolean)*
  关闭/开启当前线程上下文的虚拟线程调度器，仅对当前线程内的VT有效
  用法：相同线程内的两个VT需要并发访问共享时，其中一个VT关闭调度器，访问完共享资源后，重新开启调度器，类似Mutex的用法

  **重要：在访问共享资源完毕后必须及时开启调度器，否则其他VT将无法获得CPU资源**
  **重要：此保护生效范围仅为当前线程下的其他VT，对于其他线程的VT或者Task，请使用下面的mutex api或者lock语法**


上层RTOS互斥锁语法糖（仅当RTOS环境可用，请勿在同线程中的VT之间使用）：
