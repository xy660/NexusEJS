declare global {
  ...
  function runTask(entry: Function): NexusRTOSTask;
  function vtStart(entry: Function): void;
  function vtDelay(ms: number): void;
...
declare class Serial {
     ...
     * @param rxPin RX引脚
     * @param baudRate 波特率
     * @param dataBits 数据位 (5,6,7,8)，默认8
     * @param parity 校验位 (0:none,1:even,2:odd)，默认0
     * @param stopBits 停止位 (1,2,1.5)，默认1
     * @param flowControl 流控制 (0:none,1:RTS,2:CTS,3:RTS|CTS)，默认0
     * @returns Serial实例
     */
    static init(
        txPin: number,
     ...
     * @returns Serial实例
     */
    static initDefault(baudRate: number): SerialInstance;
}
...
interface SerialInstance {
    /**
     * 写入数据
     * @param data 要写入的数据，可以是数字、字符串或Buffer
     * @returns 写入是否成功
     */
    write(data: number | string | Buffer): boolean;
     ...
     * @returns 实际读取的字节数，负数表示失败
     */
    read(buffer: Buffer, length: number, timeout?: number): number;

    /**
     * 读取一个字节
     * @returns 读取的字节值(0-255)，-1表示无数据
     */
    readByte(): number;

    /**
     * 读取一行（直到遇到换行符或超时）
     * @returns 读取到的字符串（不包含换行符）
     */
     ...
     * @param parity 校验位 (0:none,1:even,2:odd)
     * @param stopBits 停止位 (1,2,1.5)
     * @returns 设置是否成功
     */
    setConfig(
        baudRate: number,
